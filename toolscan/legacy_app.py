...
def check_one_website(index: int, website: dict[str, Any]) -> tuple[int, dict[str, Any]]:
    ...
    for url in candidate_urls:
        try:
            ...
            public_ip = resolve_public_ip(domain)
            return index, {
                'domain': domain,
                'public_ip': public_ip,
...
def fetch_best_solution_html(session: Session, final_url: str, fallback_url: str) -> str:
    ...
    for url in candidates:
        try:
            resp = session.get(url, timeout=SOLUTION_FORM_TIMEOUT, allow_redirects=True)
    ...
    if 'html' in content_type:
        ...
        if form_info is not None:
            ...
            try:
                ...
                append_login_debug(debug_steps, 'BROWSER', 'Form submit lỗi, chuyển sang browser fallback.')
...
def attempt_json_login(session: Session, base_url: str, html: str, username: str, password: str, debug_steps: list[str] | None = None) -> Response | None:
    ...
    raw_endpoints = strategy.get('endpoints') or []
    deduped_endpoints: list[str] = []
    for endpoint in raw_endpoints:
        if endpoint not in deduped_endpoints:
            deduped_endpoints.append(endpoint)
    endpoints = deduped_endpoints[:max(1, JSON_LOGIN_MAX_ENDPOINTS)]

    raw_payload_variants = strategy.get('payload_variants') or []
    deduped_payloads: list[dict[str, Any]] = []
    seen_payload_signatures: set[tuple[str, ...]] = set()
    for template in raw_payload_variants:
        signature = tuple(sorted(template.keys()))
        if signature in seen_payload_signatures:
            continue
        seen_payload_signatures.add(signature)
        deduped_payloads.append(template)
    payload_variants = deduped_payloads[:max(1, JSON_LOGIN_MAX_PAYLOADS)]

    append_login_debug(debug_steps, 'JSON', 'Phát hiện chiến lược JSON login.', endpoints=endpoints, redirects=strategy.get('redirect_candidates'), payload_variants=[list(item.keys()) for item in payload_variants])
    ...
    for endpoint in endpoints:
        for template in payload_variants:
